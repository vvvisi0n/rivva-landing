...
export function EnableNearby(props: {
  ...
  const { state, request } = useGeo();

  return (
    <div className="rounded-2xl border border-zinc-200 p-5">
      <div className="text-sm font-semibold">Enable nearby discovery</div>
      <div className="mt-2 text-sm text-zinc-600">
        Anchyr uses your location to show people and activities near you. We
        never show an exact address.
      </div>
      ...
      {state.status === "denied" && (
        <div className="mt-3 text-sm text-red-600">
          Location permission denied. You can continue with city-only mode.
        </div>
      )}

      {state.status === "error" && (
        <div className="mt-3 text-sm text-red-600">{state.message}</div>
      )}
      ...
      {state.status === "ready" && (
        <div className="mt-3 flex items-center justify-between rounded-xl bg-zinc-50 p-3 text-sm">
          ...
          <button
            onClick={() => props.onCoords(state.lat, state.lng)}
