...
import NewUserSpotlight from "@/components/NewUserSpotlight";
...
export default function DiscoverPage() {
  ...
  useEffect(() => {
    if (!newId) return;
    const el = document.getElementById(`match-${newId}`);
    if (el) {
...
const [i, setI] = useState(0);
  ...
  useEffect(() => {
    const ids = feed.filter((m) => isLiked(m.id)).map((m) => m.id);
    setLikedIds(ids);
  }, [feed]);

  useEffect(() => {
    setAutoSpeakOn(getLumiAutoSpeak());
    const onStorage = () => setAutoSpeakOn(getLumiAutoSpeak());
    window.addEventListener("storage", onStorage);
    return () => window.removeEventListener("storage", onStorage);
  }, []);

  const current: Match | undefined = feed[i];

  function onPass() {
    if (i < feed.length - 1) setI((prev) => prev + 1);
  }
        ...
        <section className="max-w-3xl mx-auto">
          <header className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold">Discover</h1>
              <p className="text-white/60 text-sm mt-1">
                Explore profiles and see why they align with you.
              </p>
            </div>
